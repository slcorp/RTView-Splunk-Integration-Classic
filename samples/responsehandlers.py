    ...
    def __call__(self, response_object,raw_response_output,response_type,req_args,endpoint):
        if response_type == "json":        
            ...
            for rtvDataRow in parsedJson["data"]:
                rtvDataRow["Timestamp"] = datetime.datetime.fromtimestamp(rtvDataRow["time_stamp"]/1000).strftime('%d-%b-%Y %H:%M:%S')
                #if "Expired" in rtvDataRow:
                #    if rtvDataRow["Expired"] == False:
                #        del rtvDataRow["Expired"]
